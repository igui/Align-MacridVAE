...
def kl_loss(mu, logvar):
    return torch.mean(0.5 * torch.sum(torch.exp(logvar) + mu**2 - 1 - logvar, dim=1))
...
class MultiDAE(nn.Module):
    def __init__(self, M, D, dropout):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.M, self.H), nn.Tanh(), nn.Linear(self.H, self.D), nn.Tanh()
        )
        self.decoder = nn.Sequential(
            nn.Linear(self.D, self.H), nn.Tanh(), nn.Linear(self.H, self.M)
        )
    ...
    def loss_fn(self, X, X_logits, X_mu, X_logvar, A, A_logits, A_mu, A_logvar, anneal):
...
class MultiVAE(nn.Module):
    def __init__(self, M, D, dropout):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.M, self.H), nn.Tanh(), nn.Linear(self.H, self.D * 2)
        )
        self.decoder = nn.Sequential(
            nn.Linear(self.D, self.H), nn.Tanh(), nn.Linear(self.H, self.M)
        )
...
class DisenVAE(nn.Module):
    ...
    def encode(self, X, cates):
        n = X.shape[0]
        X = self.drop(X)
        X = X.view(n, 1, self.M) * cates.t().expand(n, self.K, self.M)  # n * K * M
    ...
    def decode(self, z, items, cates):
        ...
        probs = torch.sum(
            probs.view(n, self.K, self.M) * cates.t().expand(n, self.K, self.M), dim=1
        )
...
class Aligner(nn.Module):
    def __init__(
        ...
        self.visual_proj = nn.Sequential(
            nn.Linear(input_dimension_visual, embedding_dimension, bias=True),
            # nn.Tanh()
        )
        self.textual_proj = nn.Sequential(
            nn.Linear(input_dimension_textual, embedding_dimension, bias=True),
            # nn.Tanh()
        )
        self.tau = nn.Parameter(torch.tensor(0.15, dtype=torch.float32))
    ...
    def fit(self, items_textual, items_visual, batch_size=128, epochs=10):
        ...
        dataset = EmbedDataset(items_textual=items_textual, items_visual=items_visual)
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=batch_size, shuffle=True
        )

        self.train()
        optimizer = torch.optim.Adam(self.parameters(), weight_decay=0.01, lr=0.001)
...
class DisenEEVAEMulti(DisenVAE):
    def __init__(self, M, K, D, tau, dropout, items_visual, items_textual, init=True, device="cuda"):
        ...
        if not init:
            return # We don't need to align (when loading a pretrained models)
        ...
        items_textual, items_visual = aligner.transform(
            items_textual=items_textual, items_visual=items_visual
        )
    ...
    def forward(self, X, A):
        ...
        return logits, mu, logvar, logits_visual, logits_textual, None
